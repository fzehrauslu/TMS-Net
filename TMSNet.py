...
class FeatureAdaptation(nn.Module):
    def __init__(self,InputChannelNoEncoder,InputChannelNoDecoder,W,Without_HighFrequency=False,Activation= F.relu,KernelSize=3,Normalisation="Group",NormalisationFirst=False):
        ...
        self.Unet_Layer8E= ResidualLayer(InputChannelNoEncoder,InputChannelNoDecoder,KernelSize, Normalisation,W, Activation,NormalisationFirst)
        if  Without_HighFrequency!=True:
            self.D1_1=ResidualLayer(3*InputChannelNoEncoder,3*InputChannelNoDecoder,KernelSize, Normalisation,W, Activation,NormalisationFirst)
    ...
    def forward(self,x1,x1_H):
        ...
        if len(x1_H)>0:
            ...
            x1_H[0]=self.D1_1(x1_H[0].view(b,-1,w,h)).view(b,self.OutChannel,3,w,h)
...
class ResidualLayer(nn.Module):
    def __init__(self,InChannelNo,OutChannelNo,KernelSize, Normalisation,W, Activation, NormalisationFirst=True):
        super(ResidualLayer, self).__init__()
        self.Layer1= ConvLayer(InChannelNo,OutChannelNo,KernelSize, Normalisation,W, Activation,NormalisationFirst)
        self.Layer2= ConvLayer(OutChannelNo,OutChannelNo,KernelSize, Normalisation,W, Activation,NormalisationFirst)
        
        self.resLayer=ConvLayer(InChannelNo,OutChannelNo,1, Normalisation,W, Activation,NormalisationFirst)
        self._initialize_weights()
